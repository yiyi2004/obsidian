...
1. 根据 g 查询 所有 p 的列表 node=g query_type=1
2. 根据 g 查询 所有 g.p.a 的列表 node=g query_type=2
3. 根据 g.p 查询 所有 g.p.a 的列表 node=g.p query_type=3

> 设计 node 操作通用结构体

- 在 src/common/stree.go 添加结构体

## 10 编写 query_type=1 的查询 根据 G 查询 所有 P 的列表

- 先查询 g ，再查 path=/g.id 的 p
- 注意查询 p 需要查询多个，用到 StreePathGetMany 函数
...
## 11 编写 query_type=2 的查询 根据 G 查询 所有 g.p.a 的列表

- 编写 query_type=2 的查询 根据 g 查询 所有 g.p.a 的列表
- 先查 g ，再查 p 最后查 a ，中间有一步没有都返回空
...
## 12 编写 query_type=3 的查询 根据 g.p 查询 所有 g.p.a 的列表 node=g.p query_type=3
...
> 强制删除 g 的时候，分两步
> 
> - 第一步 删除 path 前缀的 p 和 a del_where="path like '/1/%' and level in(2,3) "
> - 第二步 删除这个 g
...
- 可以通过先删除后增加变向支持
- 修改的时候如果使用 nodename 拼接的 path 那么涉及到 nodename 不能修改的问题 [issue](https://github.com/ning1875/stree-index/issues/2)
