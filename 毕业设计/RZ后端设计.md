...
1. 问牛需不需要一个界面，RZPZ 资产测绘。
2. 问场景构建组的人需要哪些需求。
3. 前端的具体情况，需要他们整理需求。
4. 后端具体的情况，后端一共 3 个人 (周、张、栾)
5. 前端的人并不多，构建网络的拓扑。自动化的决策。
6. 需要有一个公共的区域去同步需求。
7. 后端的 API 需要不断的测试。
8. 栾慎凯 —— 让他在服务器上搭建环境。  
9. 需要知道基础的策略是什么 —— 需要问算法组那边要哦。
10. 信息搜集还是要的，需要用到 prometheus。将监控得到的攻击者相关的数据，输入到模型中。日志数据要怎么处理呢？？？
11. 如何维护网络拓扑。
12. 多个攻击者怎么办呢
