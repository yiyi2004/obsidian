...
## 02 管理 Task 的 Taskmap

- 新建 taskjob\tasks.go ，定义管理 task 的 taskmap
...
- 分配任务，首先从本地 map 中获取任务
- 如果找到了就更新任务的动作，比如 kill 任务
- 然后启动新的任务，将 task 塞入 map 中管理
...
- 在 agent 的配置文件中写上 meta 的字段,config\config.go
...
- open-devops-agent.yml 中填写相关的 job 配置
...
- 完成这个周期性 rpc 交互的任务
- 构造 TaskReport 的 rpc 请求，其中 ReportTasks 来自本地 map 的任务收集任务
- 遍历 rpc 的结果，分配任务
