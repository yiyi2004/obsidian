...
- 对于处于 Level 1 阶段的组织机构而言，使用 ATT&CK 进行检测分析的第一步，就是了解组织机构拥有哪些数据以及检索数据的能力。毕竟，要找到可疑的行为，需要能够看到系统上发生了什么。一种方法是查看每个 ATT&CK 技术列出的数据源。通过这些数据源信息可以知道，需要收集哪些类型的数据才能检测到相应的技术。
