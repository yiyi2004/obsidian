...
- 在common中定义相关的探测metrics ，位置common\xprober_metrics.go
...
- 所以还需要在rpc上新增相关的PushProberResults方法
- 位置D:\go_path\src\open-devops\src\modules\agent\rpc\xprober.go
...
- 获取region和ip的方法，位置xprober\get_metadata.go
...
- config中添加个region字段，用于指定region

```go
	Region string `yaml:"region"`
```

- manager init代码,位置 xprober\target.go
