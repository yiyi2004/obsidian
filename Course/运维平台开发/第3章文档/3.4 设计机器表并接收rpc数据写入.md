...
- 结构体使用 json.RawMessage
- 表结构中使用 `tags` varchar(1024) DEFAULT '' COMMENT ' 标签 map',
...
> server 的 rpc 方法接收到 agent 上报信息中应该做增量更新

- D:\go_path\src\open-devops\src\modules\server\rpc\agent_info.go
- 先获取这个 uid 的记录
- 如果没有就插入，需要算哈希值
- 如果有，还要判断哈希值或者 update_time 相同不，不相同就更新
