...
- [ ] Python API Flask，API + 有验证的文件服务器 | 这个实现起来还是比较简单的 | **1 天左右时间**
	- [ ] 需要一个数据库存储持久化存储模型的信息
	- [ ] redis 存储临时信息 | 临时信息
	- [ ] 存储客户端上传的目标图片 | 音频 以及一些基本信息，基本信息要以什么形式存在呢？json ---> 但似乎需要用户自己创建了
		- [ ] 给上传的文件起一个名字，如果没有名字，会创建一个名字，这个名字最好不是随机的，因为随机的不好记忆和获取 —— 这之后就是开始训练的过程了 | 取名字是随机的，在初始化的时候为每个名字创建一个 hash 表
		- [ ] 不同版本的模型对应相同的人的生成的密钥很**大概率**是不同的
		- [x] 数据库表设计 ✅ 2023-07-09
		- [ ] 如何表示训练的进度呢？通过训练轮数的数值。—— 两份部分 | 一个进度条 | 或者返回中间的指标以及最终的指标 | 可以在中间增加一个**消息队列**
