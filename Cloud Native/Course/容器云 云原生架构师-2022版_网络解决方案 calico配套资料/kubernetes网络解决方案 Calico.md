## Kubernetes 网络解决方案 Calico

## 一、CNI 方案

基础原理、方案解读、小结 三个方面来学习
...
![image-20220903022845285](image/image-20220903022845285.png)
...
![image-20220905120240844](image/image-20220905120240844.png)
...
![image-20220722175645610](image/image-20220722175645610.png)
...
![image-20220722180040443](image/image-20220722180040443.png)
...
![architecture-calico](image/architecture-calico.svg)
...
| 组件名                 | 组件作用                                                     |
| ---------------------- | ------------------------------------------------------------ |
| calico-node            | 需要部署到所有集群节点上的代理守护进程，提供封装好的 Felix 和 BIRD |
| calico-kube-controller | 专用于 k8s 上对 calico 所有节点管理的中央控制器。负责 calico 与 k8s 集群的协同及 calico 核心功能实现。 |
...
环境创建完毕后，会生成一个 tunl0 的网卡，所有的流量会走这个 tunl0 网卡
...
这一节，我们从 bgp 改造、反射器实践、小结 三个方面来学习
...
![image-20220722193530812](image/image-20220722193530812.png)
...
这一节，我们从 ip 限制实践、pod 限制实践、小结 三个方面来学习。
