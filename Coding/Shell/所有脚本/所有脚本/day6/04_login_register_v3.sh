...
write_user_to_file(){
  ...
  register_phone=$3
  register_email=$4
  user_status=$(grep ${register_user} ${passwd_file} >/dev/null 2>&1 && echo "false" || echo "true")
  if [ "${user_status}" == "true" ] 
  then
    echo "${register_user} ${register_passwd} ${register_phone} ${register_email}" >> ${passwd_file}
    echo "用户 ${register_user} 注册成功!!!"
  else
    echo "用户 ${register_user} 已存在，请重试"
    break
  fi
}
...
user_login_func(){
  # 获取用户的信息
  read -p "请输入登录用户: " login_user
  user_status=$(check_func ${register_user} ${user_rule})
  if [ "${user_status}" == "true"  ]
  then
    # 获取用户状态
    login_status=$(grep -w ${login_user} ${passwd_file} >>/dev/null 2>&1 && echo "true" || echo "false" )
    if [ "${login_status}" == "true" ] 
    then
      file_user=$(grep ${login_user} ${passwd_file} | cut -d " " -f 1)
      file_pass=$(grep ${login_user} ${passwd_file} | cut -d " " -f 2)
      read -p "请输入登录密码: " login_pass
      if [ "${login_user}" == "${file_user}" -a "${login_pass}" == "${file_pass}" ]
      then
         echo -e "\e[31m登录成功!!! 欢迎 ${login_user} 回来"
      else
         echo -e "\e[31m登录密码有误，请重试。"
      fi
    else
      echo "用户不存在，不可以后续操作"
    fi
  else
    echo "您输入的用户名格式有误，请重试!!!"
  fi
}
# 用户注册检测逻辑
user_register_func(){
     ...
     then
        ...
        then
           ...
           then
              ...
              if [ "${ack_status}" == "yes" ]
              then
                 write_user_to_file ${register_user} ${register_passwd} ${register_phone} ${register_email}
              elif [ "${ack_status}" == "no" ]
              then
