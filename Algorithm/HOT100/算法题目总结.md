...
pre[j] - pre[i] = k  
pre[i] = pre[j] - k ---> pre[i] 存在，那么就存在 子序列 == k  
这个时候用 map 去记录 == pre[i] 存在的数量。  
别忘记 help[0] 需要初始化为 1

> 前缀和的变形题，可以考虑移项将公式进行转化。
...
![[Snipaste/Pasted image 20240121192807.png]]
...
- [单调队列 滑动窗口最大值【基础算法精讲 27】_哔哩哔哩_bilibili](https://www.bilibili.com/video/BV1bM411X72E/?vd_source=25509bb582bc4a25d86d871d5cdffca3)
...
如果当前数字 + 之前的前缀和 > 当前数字，nums[i] = nums[i] + nums[i-1]
...
- 遍历数组，不断更新 minPrice 和 maxPrice

### 55. 跳跃游戏

- 遍历数组，不断更新能跳到的最远位置
- if k(最远位置) < i(当前位置) ---> return false

### 45. 跳跃游戏 II

- 从末尾开始遍历数组，记录能跳到 targetIndex 的离终点最远的位置
- 不断更新 targetIndex
- targetIndex == 0 是循环退出条件
...
- 不要死脑筋，循环之中还能再放循环
- 内层循环遍历完全平方数，dp[i] = min(minVal, dp[i-j*j]+1)
- 还是比较简单的 dp

### 322. 零钱兑换

- 背包问题，先遍历硬币，再遍历背包
- 初始化比较特殊，初始化为 math.MaxInt
- 最后判断 dp[amout] == math.MaxInt，返回 -1

### 139. 单词拆分

- 要根据 dp 递归关系式进行初始化，**要简单的推几个值，就会发现问题。**
