## Server 端和 Agent RPC 通信

### Server 端开启 Rpc Server
...
- 是一种高效的二进制序列化格式。它允许你在多种语言 (如 JSON) 之间交换数据。但它更快更小。

> main 函数中启动 rpc server

- g.Add 添加函数
- 长时间运行的 http 和 rpc 用 error chan 在 g.add 中做
- 因为 rpc 和 http 中不方便接收 ctxAll.Done 的信号，所以呢只能再 g.Add 中 select 接收
...
#### 如果在 Rpcserver 启动的时候不处理 ctxAll.Done

- 先把 rpc server 启动的 g.Add 改造如下
...
- 启动服务后点击 stop，发现进程并未退出
- 在 linux 上 telnet win 上的 8080 端口发现还可以通，说明 rpc server 根本没关掉
...
- 符合我们的预期：rpc 报错，通知接收 signal 的 go 退出
...
- rpc/cli.go
- 封装一个 RpcCli 结构体，将 logger 和 serverAddr 和 rpccli 封装在一起
- 每次 call 之前都 GetCli 一下，避免 cli 不可用
...
- agent main 中添加初始化 rpc client 并调用的方法
